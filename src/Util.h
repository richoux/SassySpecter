...
namespace Util
{
    ...
    bool            UnitCanMetaTypeNow(const Unit & unit, const UnitType & type, SassySpecterBot & m_bot);
    UnitType        GetTownHall(const CCRace & race, SassySpecterBot & bot);
    UnitType        GetRefinery(const CCRace & race, SassySpecterBot & bot);
    UnitType        GetSupplyProvider(const CCRace & race, SassySpecterBot & bot);
...
#ifdef SC2API
    sc2::BuffID     GetBuffFromName(const std::string & name, SassySpecterBot & bot);
    sc2::AbilityID  GetAbilityFromName(const std::string & name, SassySpecterBot & bot);
#endif
